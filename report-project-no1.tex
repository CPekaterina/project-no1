...
\begin{enumerate}
	...
	\begin{equation}
	\tilde{a}[0]=a[0],\quad \tilde{a}[i]=a[i]-b[i-1]\dfrac{c[i-1]}{\tilde{a}[i-1]}
	\label{eq:3}
	\end{equation}
	...
	\item Accordingly $w[]$ is substituted by
		\begin{equation}
	\tilde{w}[0]=a[0],\quad \tilde{w}[i]=w[i]-\tilde{w}[i-1]\dfrac{c[i-1]}{\tilde{a}[i-1]}
	\label{eq:4}
	\end{equation}
	...
	\item Finally backward substitution is used to gain the result for the unknown vector $v$ which is stored in another array $v[]$:
		\begin{equation}
	v[n-1]=\dfrac{\tilde{w}[n-1]}{\tilde{a}[n-1]},\quad v[i]=\dfrac{\tilde{w}[i]-b[i+1]\cdot v[i+1]}{\tilde{a}[i-1]}
	\label{eq:5}
	\end{equation}
